...
class Agent(nn.Module):
    ...
    Keyword Args:
        ...
        n_step: N-step return horizon for TD targets. n=1 is the standard 1-step Bellman
            backup. Larger values sum over multiple future rewards before bootstrapping.
            Truncated at episode boundaries and sequence padding. Defaults to 1.
    """
    ...
    def _reduce_critic_ensemble(self, q_ensemble: torch.Tensor):
    ...
    def forward(
        ...
        if not self.fake_filter or self.online_coeff > 0:  # if we use the critic to train the actor
            ...
            with torch.no_grad():
                ...
                # Reduce critic ensemble first, then apply n-step Bellman.
                # For n>1 we must reduce first because we need a single
                # Q-value per position to bootstrap from at arbitrary offsets.
                # For n=1 this is mathematically equivalent to applying Bellman
                # first then reducing (both min and mean commute with r + γ(1-d)·Q
                # since r,γ,d are constant across critics).
